          ...
          <div style="position:relative;width:96px;margin:0 auto 14px;">
            @if (previewUrl() || user()!.profileImageUrl) {
              <img [src]="previewUrl() || apiBase + user()!.profileImageUrl"
                style="width:96px;height:96px;border-radius:50%;object-fit:cover;border:3px solid var(--danger);" />
            } @else {
              <div style="width:96px;height:96px;border-radius:50%;background:var(--danger);color:#fff;display:flex;align-items:center;justify-content:center;font-size:2.5rem;font-weight:700;">
                {{ user()!.name[0].toUpperCase() }}
              </div>
            }
            <label style="position:absolute;bottom:0;right:0;width:30px;height:30px;background:var(--danger);border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.2);" title="Change photo">
