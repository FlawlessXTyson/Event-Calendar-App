...
@Component({
  ...
  template: `
    <div style="max-width:600px;">
      <div style="margin-bottom:24px;"><h1 style="font-size:1.5rem;">My Profile</h1><p>Update your account information</p></div>
        ...
        <div class="card card-body" style="margin-bottom:20px;text-align:center;">
          <div style="position:relative;width:96px;margin:0 auto 14px;">
            @if (previewUrl() || user()!.profileImageUrl) {
              ...
              <div style="width:96px;height:96px;border-radius:50%;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:2.5rem;font-weight:700;">
                {{ user()!.name[0].toUpperCase() }}
              </div>
            }
            <!-- Upload overlay -->
            <label style="position:absolute;bottom:0;right:0;width:30px;height:30px;background:var(--primary);border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.2);" title="Change photo">
          ...
          <div style="margin-top:10px;"><span class="badge badge-primary">{{ roleLabel(user()!.role) }}</span></div>
